...
def _process_tfidf(in_queue: Queue, out_queue: Queue) -> None:
    """
    Worker for the TFIDF vectorization

    :param in_queue:
    :param out_queue:
    :return:
    """
    vectorizer: TfidfVectorizer = pickle.load(file=open(_args.model, 'rb'))
    ...
    while True:
        ...
        v = vectorizer.transform(gists)
        out_file = os.path.join(_args.features, '{}.vecs'.format(uuid))
        with open(out_file, 'wb') as out:
            pickle.dump(v, out)

        out_queue.put(out_file)


def vecs_tfidf():
    """
    Produce TFIDF vectors for a collection of texts. In the folder args.csvpath, it opens each CSV file and uses the
    column name args.tag to get the texts. Runs in parallel woth args.nbworkers workers
    It saves a file <opinion_id>.vecs for each opinion_id.

    :return:
    """
    # Load the data
    data: pd.DataFrame = pd.read_json(_args.gists, orient='records', lines=True)

    # Create the input iterator
    def _iter_gists():
        for cited_opinion_id, gists in data.groupby('cited_opinion_id'):
            yield {'uuid': cited_opinion_id, 'gists': gists['gist']}

    _ = multiprocess_with_queue(worker_fn=_process_tfidf, input_iterator_fn=_iter_gists,
                                nb_workers=_args.nbworkers, description='TFIDF')
...
def parse_args(argstxt=None):
    ...
    subparser_vecs = parser_vecs.add_subparsers(title='Sub-Sub commands', description='Valid vecs subcommands')

    parser_vecs_bert = subparser_vecs.add_parser('bert')
    parser_vecs_bert.add_argument('--cache_tokens', type=str, help='File with cached tokens. '
                                                                   'Generated by the tokens command')
    parser_vecs_bert.add_argument('--batch_size', type=int, help='Batch size for inference')
    parser_vecs_bert.add_argument('--features', type=str, help='Path of the folder where the features will be stored')
    parser_vecs_bert.set_defaults(func=vecs_bert)

    parser_vecs_tfidf = subparser_vecs.add_parser('tfidf')
    parser_vecs_tfidf.add_argument('--model', type=str, help='Path to a pickled TFIDF vectorizer')
    parser_vecs_tfidf.add_argument('--gists', type=str, help='Path to the gist JSON file')
    parser_vecs_tfidf.add_argument('--features', type=str, help='Path of the folder where the features will be stored')
    parser_vecs_tfidf.add_argument('--nbworkers', type=int, help='Number of parallel workers')
    parser_vecs_tfidf.set_defaults(func=vecs_tfidf)
